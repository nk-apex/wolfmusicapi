...
export async function searchSongs(query: string) {
  try {
    ...
    log(`Search error: ${error.message}`, "scraper");
    throw error;
  }
}
...
export async function checkVideo(videoId: string) {
  try {
    ...
    log(`Check error: ${error.message}`, "scraper");
    throw error;
  }
}
...
async function tryGetDirectDownload(videoId: string, format: "mp3" | "mp4"): Promise<string | null> {
  try {
    ...
    const res = await fetch(converterApis[0], {
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
    ...
    log(`Direct download attempt failed: ${e.message}`, "scraper");
  }
  return null;
}
...
export async function getDownloadInfo(url: string, format: "mp3" | "mp4" = "mp3") {
  ...
  try {
    ...
    let downloadUrl = await tryGetDirectDownload(videoId, format);
    ...
    log(`Download info error: ${error.message}`, "scraper");
    return {
      success: false,
